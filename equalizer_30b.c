...
#include "equalizer_presets_30b.h"
...
#define PRESET_TEXT N_( "Equalizer preset 30b" )
#define PRESET_LONGTEXT N_("Preset to use for the equalizer 30b." )
...
#define VLC_BANDS_TEXT N_( "Use Customized frequency bands" )
...
vlc_module_begin ()
    set_description( N_("Equalizer with 30 bands") )
    set_shortname( N_("equalizer30b" ) )
    ...
    add_string( "equalizer-preset", "flat", PRESET_TEXT,
        ...
        change_string_list( preset_list_30b, preset_list_30b_text )
                       ...
                       eqz_config_t *p_eqz_config )
{
    ...
    const float *f_freq_table_30b = f_vlc_frequency_table_30b;
    ...
    p_eqz_config->i_band = EQZ_BANDS_30;

    for( int i = 0; i < EQZ_BANDS_30; i++ )
    {
        float f_freq = f_freq_table_30b[i];
                         ...
                         vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    for( unsigned i = 0; i < NB_PRESETS; i++ )
        if( !strcasecmp( eqz_preset_30b[i].psz_name, psz_preset ) )
        {
            preset = eqz_preset_30b + i;
            break;
        }
    ...
    if( preset == NULL )
    {
        ...
        for( unsigned i = 0; i < NB_PRESETS; i++ )
             msg_Info( p_aout, "  - '%s'", eqz_preset_30b[i].psz_name );
        return VLC_EGENERIC;
    }

    char *bands = NULL;

    for( unsigned i = 0; i < EQZ_BANDS_30; i++ )
    {
        char *psz;
